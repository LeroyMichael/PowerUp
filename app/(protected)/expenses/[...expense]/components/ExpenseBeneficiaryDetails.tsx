...
export default function ExpenseBeneficiaryDetails({
  ...
  async function callCustomerLists(
    merchant_id: string,
    currentPage: number,
    searchParam: string
  ) {
    const tempList = await getContacts(
      merchant_id,
      currentPage,
      searchParam,
      tab
  ...
  }, [session?.user.merchant_id, currentPage, search, tab]);

  return (
    <Card>
      <CardHeader>
        <CardTitle>Beneficiary Details</CardTitle>
        <CardDescription>Add Beneficiary Details</CardDescription>
      </CardHeader>
      <Separator />
      <CardContent>
        ...
        <Tabs value={tab} onValueChange={setTab} className="space-y-4 pt-2">
          <TabsList>
        ...
        {selectedCustomer && (
          <div className="mb-4">
            ...
            <CustomerCard isSelected={true} data={selectedCustomer} />
        ...
        {errors.contact_id?.message && (
          <p className="text-red-500">Please select Beneficiary</p>
        )}
